# Plan: Analysis & ADR for `distributed/` Architecture Redesign (Revised)
...
4. **Lease contract** — Define token-based leases:
   ...
   - `releaseLease(token): Promise<void>` clears the lease and updates task status via `completeTask`/`failTask`.
   - `completeTask(taskId, token, result)` and `failTask(taskId, token, error)` validate the token; if the token is missing, expired, or revoked, the call is rejected/no-op and logged.
   - `revokeStaleLeases(ttlMs)` is called by the coordinator/poller to requeue tasks whose leases have expired.
5. **WorkerPool contract** — Promote `WorkerPool` to first-class:
   ...
   - `ExternalWorkerPool.run` emits the job ID and calls `pollJobToCompletion`.
...
- **Churn gate**: If the facade PR changes more than **8 non-test call sites** that import `QueueAdapter`, or requires changes to public CLI flag parsing/usage strings, escalate to Option B.
