...
export async function runDistributedWideSearch({
  ...
  const perTaskMaxResults =
    plans.length === 0 || profile.startsWith('fixture')
      ? undefined
      : Math.ceil(maxResultsForDepth(searchDepth) / plans.length);
    ...
    if (distributed.resumeJobId) {
      ...
      for (const task of job.tasks) {
        if (task.status === 'running') {
          if (!adapter.requeueTask) throw new Error('Queue adapter cannot requeue resumed tasks');
          task.attempts = Math.max(0, task.attempts - 1);
          await adapter.requeueTask(task);
        }
      }
    } else {
