...
describe("costs", () => {
  ...
  test("checkBudget throws when provider calls exceed limit", () => {
    ...
    expect(() => checkBudget("mock", metrics, { maxProviderCalls: 2 })).toThrow(BudgetExceededError);
  });

  test("checkEstimatedBudget throws when estimated cost exceeds budget", () => {
    const estimate = estimateRunCost("tavily", "standard");
    expect(() => checkEstimatedBudget(estimate, { maxCostUsd: 0.001 })).toThrow(BudgetExceededError);
  });
