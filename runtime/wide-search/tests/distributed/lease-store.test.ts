...
describe('MemoryLeaseStore', () => {
  ...
  test('claimLease returns a unique token', async () => {
    ...
    const token = await store.claimLease('job-1-task-0001', 'worker-1', 1000);
  ...
  test('renewLease extends a valid lease', async () => {
    const store = await createStore();
    const token = await store.claimLease('job-1-task-0001', 'worker-1', 50);
  ...
  test('renewLease rejects an expired lease', async () => {
    const store = await createStore();
    const token = await store.claimLease('job-1-task-0001', 'worker-1', 10);
  ...
  test('revokeStaleLeases returns expired task ids', async () => {
    const store = await createStore();
    await store.claimLease('job-1-task-0001', 'worker-1', 10);
    await store.claimLease('job-1-task-0002', 'worker-1', 10_000);

    await new Promise((resolve) => setTimeout(resolve, 30));
    const revoked = await store.revokeStaleLeases(10);

    expect(revoked).toContain('job-1-task-0001');
    expect(revoked).not.toContain('job-1-task-0002');
  });
});
