...
import { TavilySearchProvider } from "../src/providers/tavily-provider";
import type { UsageMetrics } from "../src/types";

describe("TavilySearchProvider", () => {
  test("mock mode returns deterministic sources without an API key", async () => {
    process.env.TAVILY_MOCK = "1";
    const provider = new TavilySearchProvider("");
    const sources = await provider.search({ objective: "test query", depth: "standard", maxResults: 5 });

    expect(sources.length).toBeGreaterThan(0);
    expect(sources[0].id).toBe("TAVILY-001");
    expect(sources[0].discoveredBy).toInclude("tavily");

    delete process.env.TAVILY_MOCK;
  });
  ...
  test("increments usage metrics", async () => {
    ...
    const provider = new TavilySearchProvider("", metrics);
  ...
  test("respects maxResults", async () => {
    process.env.TAVILY_MOCK = "1";
    const provider = new TavilySearchProvider("");
    const sources = await provider.search({ objective: "limit test", depth: "standard", maxResults: 1 });
